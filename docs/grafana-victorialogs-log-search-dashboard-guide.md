...
多条件的“包含/不包含”使用官方 [substring filter](https://docs.victoriametrics.com/victorialogs/logsql/#substring-filter)，区分大小写，按字面文字匹配，不把 `.*`、括号或输入中的 `*` 当作正则或查询通配符。例如“海康”或“康门”均可命中“海康门闸入参”。旧词查询 `_msg:"海康"` 要求词边界，不能用来实现此处的“包含”。

当前插件没有为 substring filter 提供完整的原生高亮词，因此多条件中的已应用“包含”
复用页面既有 CSS Highlight API。只有 URL 表单状态经同一个构造函数生成的表达式与
实际查询一致时才着色，草稿编辑和排除条件不着色；高亮不修改正文或触发额外查询。
沿用 100 万字符、2000 范围、4096 字符单值的页面预算，刷新、虚拟滚动与清空时更新。

实际子串高亮局部（仅保留两字，不含业务正文）：

![海康子串高亮](images/vvg-substring-highlight.jpg)

顶部原生 `message` 输入框保留词/短语查询，默认 `*`。CCE、主机 Java 和 PHP 的多条件均查询 `_msg`；不改采集字段、正文映射或临时 `message` 别名。旧 URL 可以恢复表单，再点 Apply 会生成新语义；不在加载或编辑时自动触发查询。子串匹配应先通过项目、服务和时间缩小范围，保留默认 15 分钟、500 行与现有超时。
